...
Handle fGetTeamScore = INVALID_HANDLE;
Handle fClearTeamScores = INVALID_HANDLE;
...
Handle cvarTeamSwapping = INVALID_HANDLE;
Handle cvarVoteScrambling = INVALID_HANDLE;
Handle cvarFullResetOnEmpty = INVALID_HANDLE;
...
Handle mapScores = INVALID_HANDLE;
...
/* Team Placement */
Handle teamPlacementTrie = INVALID_HANDLE; //remember what teams to place after map change
...
Address g_pDirector = Address:0;
Handle fSwapTeams = INVALID_HANDLE;
Handle fAreTeamsFlipped = INVALID_HANDLE;
Handle fRestart = INVALID_HANDLE;
...
PrepareAllSDKCalls()
{
	...
	{
		ThrowError("Could not load gamedata/l4d2scores.txt");
	}

	// GetTeamScores
	StartPrepSDKCall(SDKCall_GameRules);
	if(PrepSDKCall_SetFromConf(gConf, SDKConf_Signature, "CTerrorGameRules::GetTeamScore"))
	{
	...
	// ClearTeamScores
	StartPrepSDKCall(SDKCall_GameRules);
	if(PrepSDKCall_SetFromConf(gConf, SDKConf_Signature, "CTerrorGameRules::ClearTeamScores"))
	{
	...
	PrepSDKCall_SetFromConf(gConf, SDKConf_Signature, "SurvivorBot::SetHumanSpectator");
	PrepSDKCall_AddParameter(SDKType_CBasePlayer, SDKPass_Pointer);
	fSHS = EndPrepSDKCall();

	StartPrepSDKCall(SDKCall_Player);
	PrepSDKCall_SetFromConf(gConf, SDKConf_Signature, "CTerrorPlayer::TakeOverBot");
	PrepSDKCall_AddParameter(SDKType_Bool, SDKPass_Plain);
	fTOB = EndPrepSDKCall();
	...
	g_pDirector = GameConfGetAddress(gConf, "CDirector");
	...
	if(!PrepSDKCall_SetFromConf(gConf, SDKConf_Signature, "CDirector::RestartScenarioFromVote"))
	{
	...
	if(!PrepSDKCall_SetFromConf(gConf, SDKConf_Signature, "CDirector::SwapTeams"))
	{
	...
	if(!PrepSDKCall_SetFromConf(gConf, SDKConf_Signature, "CDirector::AreTeamsFlipped"))
	{
...
stock L4D2_RestartScenarioFromVote(const char[] mapName)
{
...
public OnMapEnd()
{
	...
	*/
	TeamSwappingType swapKind = TeamSwappingType:GetConVarInt(cvarTeamSwapping);
...
CalculateNextMapTeamPlacement()
{
	...
	*/
	...
	bool AreTeamsFlipped = L4D2_AreTeamsFlipped();
...
Handle ScrambleVoteMenu = INVALID_HANDLE;
...
public Action Command_SwapMenu(client, args)
{
	...
	Handle menu = CreateMenu(Menu_SwapPanel);
...
public Action Command_GetTeamScore(client, args)
{
	...
	int team = StringToInt(arg1);
	int b1 = StringToInt(arg2);

	int score = SDKCall(fGetTeamScore, team, b1);
...
public Action Command_ClearTeamScores(client, args)
{
	if(fClearTeamScores == INVALID_HANDLE)
	{
		DebugPrintToAll("Could not load ClearTeamScores function");
		return Plugin_Handled;
	}
...
public EventVoteEndFail(Handle event, const char[] name, bool dontBroadcast)
{
