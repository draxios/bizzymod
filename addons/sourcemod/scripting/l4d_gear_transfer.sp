...
public void OnPluginStart()
{
	...
	g_hCvarDistGive =		CreateConVar(	"l4d_gear_transfer_dist_give",		"100.0",		"How close you have to be to transfer an item. Also affects bots auto give range.", CVAR_FLAGS);
	g_hCvarDistGrab =		CreateConVar(	"l4d_gear_transfer_dist_grab",		"200.0",		"How close the bots need to be for them to pick up an item.", CVAR_FLAGS);
	...
	g_hCvarIdle =			CreateConVar(	"l4d_gear_transfer_idle",			"1",			"0=No, 1=Yes. Can items be transferred with idle players, players will be able to grab and switch items with idle players.", CVAR_FLAGS);
	...
	g_hCvarTimerGive =		CreateConVar(	"l4d_gear_transfer_timer_give",		"0.5",			"0.0=Off. How often to check survivor bot positions to real clients for auto give.", CVAR_FLAGS, true, 0.0, true, 10.0);
	...
	g_hCvarTimeout =		CreateConVar(	"l4d_gear_transfer_timeout",		"1.0",			"Timeout to stop bots returning an item after switching with a player. Timeout to prevent bots auto grabbing a recently dropped item.", CVAR_FLAGS, true, 1.0);
	g_hCvarTraces =			CreateConVar(	"l4d_gear_transfer_traces",			"120",			"Maximum number of ray traces per frame for auto give/grab. This could be increased with minimal impact.", CVAR_FLAGS, true, 1.0, true, 120.0);
